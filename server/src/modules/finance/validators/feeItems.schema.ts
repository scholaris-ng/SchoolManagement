...
const feeItemBody = z.object({
  ...
  /**
   * Extra named prices for this same charge — "Zone A" transport at one
   * amount, "Zone B" at another — offered alongside the item's own `amount`
   * when a bursar raises an invoice by hand. `id` is generated by the client
   * for a new option and kept for an existing one, the same as a fee
   * structure's own line ids; omitted on a PATCH leaves the list as it is,
   * same as `paymentDestinationIds`.
   */
  priceOptions: z
    .array(
      z
        .object({
          id: z.string().uuid(),
          label: z.string().trim().min(1, 'Give this price a label').max(60),
          amount: z.coerce.number().min(0, 'An amount cannot be negative'),
        })
        .strict(),
    )
    .max(20, 'That is a lot of price options for one charge')
    .optional(),
});
